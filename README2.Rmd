...
tre <- ape::read.tree(text = readLines('https://raw.githubusercontent.com/continuousity/example_data/master/example.newick'))
plot(tre)
axis(1)
mtext('Time (My)', side = 1, line = 2)
```
...
* Site metadata file
    ...
    > We need to know a few things about sites, most importantly their goegraphic locations, and hopefully also something about sampling methodology such as area, effort, sampling technique.  The included example file (tab delimited in this case, but CSV is equally good) refers to arthropod pitfall trapping as an example
